...
int main() {
...
#else
    std::ifstream in("tangent.in");
    std::ofstream out("tangent.out");
#endif
    ...
    Circle c;
    in >> c >> p;
    long long dist = (c.center - p).norm() - sq(c.r);
    if (dist < 0) {
        out << 0 << std::endl;
    } else if (dist == 0) {
        out << 1 << std::endl;
        ...
        out << 2 << std::endl;
        p = p - c.center;
        double ca = sq(c.r) / sqrt(p.norm());
        double h2 = sq(c.r) - sq(ca);
        PointF normal(p.y, -p.x);
        normal = normal / sqrt(normal.norm());
        PointF ctr = PointF(p.x, p.y)*ca/sqrt(p.norm()) + PointF(c.center.x, c.center.y);
        out << ctr << std::endl;
        out << sqrt(p.norm()) - ca << ' ' << sqrt(h2) << std::endl;
        out << ctr + normal*sqrt(h2) << std::endl;
        out << ctr - normal*sqrt(h2) << std::endl;
    }
}
